...
class CommandCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        #   [PARTIAL_]COMMAND ...
            ...
            #  or values, depending on the type of command
                ...
                # e.g.  set  ---> ["WORKSPACE", ...]
                    ...
                    for _ in sorted_set(c.complete_keys() or []):
                        yield Completion(_, start_position=0)
        ...
        #   COMMAND [PARTIAL_](KEY ...|VALUE)
            ...
            if nargs == 1 and ts == 0:
                for _ in wfilter(sorted_set(c.complete_values() or []), t1):
                    yield Completion(_, start_position=o1)
            # when a partial key token is given, yield the list of valid ones
            # e.g.  set W  ---> ["WORKSPACE"]
            elif nargs == 2 and ts == 0:
                for _ in wfilter(sorted_set(c.complete_keys() or []), t1):
                    yield Completion(_, start_position=o1)
            # when a valid key token is given, yield the list of values
            # e.g.  set WORKSPACE  ---> ["/home/user/...", "..."]
            elif nargs == 2 and ts > 0 and t1 in c.complete_keys():
                for _ in sorted_set(c.complete_values(t1) or []):
                    yield Completion(_, start_position=0)
        # when three tokens are provided, handle format:
        #   COMMAND KEY [PARTIAL_]VALUE
        elif l == 3 and c is not None and t1 in c.complete_keys():
            if nargs == 2 and ts == 0:
                for _ in wfilter(sorted_set(c.complete_values(t1) or []), t2):
