...
class ExpiringDict(OrderedDict):
    ...
    def __getattribute__(self, name):
        if not any((
                name.startswith("_ExpiringDict__"),
                self.__locked,
                getattr(self, "_ExpiringDict__cleaned", False)
        )):
            self.__cleaned = True
            self.__cleanitems__()
    ...
    def count(self, path=None, **kwargs):
        ...
        def _rcount(d=self, a=0):
            if isinstance(d, dict):
                ...
                criteria_match = any((
                    len(kwargs) == 0,
                    all(getattr(d, attr, value) == value for attr, value in kwargs.items())
                ))
                a += 1 if criteria_match else 0
            return a
